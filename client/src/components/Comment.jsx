...
function Comment({ comments }) {
    ...
    const likeComment = async (id) => {
        try {
            await dispatch(likeTheComment(id))
            dispatch(getPostComments(postId))
        } catch (error) {
            toast.error('Error :[')
        }
    ...
    const unlikeComment = async (id) => {
        try {
            await dispatch(unlikeTheComment(id))
            dispatch(getPostComments(postId))
        } catch (error) {
            toast.error('Error :[')
        }
                            ...
                            <article key={item?._id} className="relative max-w-[960px] mx-auto py-6 text-base bg-white border-gray-200 border-l-2 pl-4 mb-6 dark:border-white-700 dark:bg-gray-800" >
                                ...
                                <div className="flex items-center mt-4 space-x-4">
                                    <button type="button" className="flex items-center gap-1 text-gray-500 dark:text-gray-400">
                                        {
                                            isAuth && item.likes?.includes(user?._id)
                                                ?
                                                <svg onClick={() => unlikeComment(item?._id)} xmlns="http://www.w3.org/2000/svg" className='w-4 h-4 dark:fill-white' viewBox="0 0 512 512"><path d="M47.6 300.4L228.3 469.1c7.5 7 17.4 10.9 27.7 10.9s20.2-3.9 27.7-10.9L464.4 300.4c30.4-28.3 47.6-68 47.6-109.5v-5.8c0-69.9-50.5-129.5-119.4-141C347 36.5 300.6 51.4 268 84L256 96 244 84c-32.6-32.6-79-47.5-124.6-39.9C50.5 55.6 0 115.2 0 185.1v5.8c0 41.5 17.2 81.2 47.6 109.5z" /></svg>
                                                :
                                        ...
                                        <div className="text-[14px] font-semibold text-zinc-500 dark:text-white/90">{item.likes?.length}</div>
                                    </button>
